...
Physiological models are developed to represent certain clinical condition common across a population of patients, or the condition of a specific patient. Therefore the structure of the model and corresponding parameters have to be identified. This information can be obtained from physiological data from data collected during physiological procedures, and/or physiological literature in which population physiological data is analyzed and summarized. Due to limitation on the interactions with the patient (e.g. during a device implantation procedure or an ablation procedure), the quantity and quality of physiological data are generally poor as there is generally not enough information to identify all the parameters in the model.  A spatio-temporal structure of the model which is similar to the conduction patterns in the heart, helps simplify the process of identifying the model parameters. A rigorous procedure for the model identification step is an important contributor to the model validation step. 
...
\subsection{Validate Models for Closed-loop Model Checking}
