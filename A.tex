...
Among all the medical devices, there are devices with both diagnostic and therapeutic functions, i.e. implantable cardiac devices, deep brain stimulation devices(\cite{Brain_sti}). These devices diagnose physiological condition of the patient and deliver therapy accordingly, which forms a closed-loop system with the patient. Their ability to (semi-) autonomously affect the physiological state of the patient makes the medical devices safety-critical, that these devices are usually classified into the highest risk category. We denote these devices as \textbf{Physiological Closed-loop Medical Devices}. 

\section{Challenges of Developing Physiological Closed-loop Medical Devices}
\subsection{Interacting With Complex Physiology}
All medical devices interacts with their environment in a closed-loop mannner. For example, an infusion pump takes operator's input and delivers fluid into the patient's body
\subsection{Limited Diagnostic and Theraputic Functions}
\subsection{Heavy Reliance on Software Control}
The diagnositic and theraputic functions of the devices are mostly controlled by their software components. 
\begin{figure}[t]
		\centering
