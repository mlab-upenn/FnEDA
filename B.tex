...
<<<<<<< HEAD
\textbf{Model complexity} is generally measured in terms of the size of state space and computation complexity of state transitions, which affect the computation cost (memory and time) for closed-loop verification. Model complexity is largely influenced by the type of interactions with the device (e.g. analog signals, sensing, actuation) and the complexity of the environment conditions specified by the physiological requirements. 
=======
\textbf{Model complexity} is generally measured in terms of the size of state space and computation complexity of state transitions, which affect the computation cost (memory and time) for closed-loop verification. Model complexity is largely influenced by the type of interactions with the device (e.g. analog signals, sensing, actuation) and the environment conditions specified but the physiological requirements. 
>>>>>>> origin/master
...
The heart models are also implemented on hardware platform (\figref{HOC}). With an analog interface the heart model can interact with a commercial pacemaker in real time. 
\begin{figure}[!t]
\center
%\vspace{-15pt}
