...
The oxygen demand of the body changes during different activities. For example, the demand is higher while running and lower while sleeping. To satisfy these demands, the heart muscles in the atria and the ventricles have to contract in accordance to optimize the blood flow. The coordinated contractions of the heart muscles are governed by the electrical conduction system of the heart (\figref{models}). A \emph{Normal Sinus Rhythm (NSR)} is the healthy heart rhythm which provides efficient blood flow. During a NSR, electrical signals are periodically generated by the \emph{Sinoatrial (SA) node} in the upper right atrium, which acts as the intrinsic pacemaker of the heart. The signals conduct throughout both atria and trigger muscle contractions to push blood into the ventricles. After a long conduction delay at the \emph{AV node} so that both ventricles are fully filled, the signals conduct through fast-conducting \emph{His-Pukinje} system to trigger almost simutanous contractions of the ventricles and pump blood out of the ventricles. Derangements from NSR can result in inefficient blood flow and thus insufficiant blood supply to the body and/or the heart itself, which are referred to as \emph{Arrythmia}. Arrythmia impair the heart's ability to efficiently pump blood and compromise the patient's health. 
...
For understanding the underlying physiological principals, heart models with over 100,000 ordinary differential equations or 4 million finite elements have been developed (\cite{cell2bed}). While these models capture in high fidelity the physiological phenomena, they do not operate in real-time or provide the appropriate interfaces for device interaction. 
