...
The heart model $H_1'$ still has equivalent locations. In Abstraction 2 we further abstract the node and path automata. With non-determinism, the node automaton react to \textsf{Act\_node!} event the same way in \textsf{RRP} and \textsf{Rest} state. In the new node automaton $N_2$ we merge the \textsf{RRP} state with the \textsf{Rest} state with:
$$N_2.Trest\_min = N_1.Trest\_min+N_1.Trrp\_min$$
$$N_2.Trest\_max = N_1.Trest\_max+N_1.Trrp\_max$$
Since during \textsf{ERP} state, the node automata won't react to any \textsf{Act\_node!} event, for a $N_1^1\| P_1\| N_1^2$ setting, the symbolic state $ER\|DO\|ER$ is equivalent to $ER\|ID\|ER$. Under the assumption that the ERP period of a node automaton is much longer than the conduction delay of a path automaton, it is guaranteed that both node automata will be in \textsf{ERP} location when the path exits the \textsf{Double} location. In the path automaton, the \textsf{Double} and \textsf{Conflict} location is merged with the \textsf{Idle} state. The location abstraction and mapping is in \figref{abs} (b). The heart can be modeled as $H_2=N_2^1\| P_2^1\| N_2^2\| P_2^2\| N_2^3$.
...
Without the ERP constraint the AV node is no longer needed and the heart can be modeled as $H_3=N_3^1\| P_3\| N_3^2$. The location abstraction and mapping is in \figref{abs} (c). The detailed proof for this timed simulation relation is in the appendix. 
\subsection{Abstraction 4: Random Heart Model (RHM)}
In Abstraction 4, we further simplify the heart model by removing the conduction path between two nodes. By setting $Trest\_min$ for both nodes to 0 the new heart model $H_4=N_3^1\|N_3^2$ covers all possible behavior of $H_3$. The location abstraction and mapping is in \figref{abs} (d). This random heart model with two nodes is the most abstract model that will be used at the beginning of the closed-loop verification process in Section 5.
